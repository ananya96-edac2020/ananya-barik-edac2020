...
select author_name from books where  author_name regexp '^[ls]';

select * from books;

-- /2.	Find all the employee name endiing with ‘ng’;/
select author_name  from books where author_name regexp 'ni$';

-- /3.	write query to find all employee job containing characters ’io’ or ‘ea’./
select author_name from books where author_name regexp 'an|ru';

-- /4.	Write a query to find all names containing characters {s,d}./

select author_name from books where author_name regexp '[a,r]';
